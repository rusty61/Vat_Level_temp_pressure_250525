...
#define BUZZER_PIN GPIO_NUM_4 // Example pin, can be changed
...
void update_motor_rate() {
  ...
  if (target_rate < 2.0 || target_rate > 400.0) {
      // Basic check if atof failed or input was non-numeric leading to 0.0,
      // or if the number is genuinely out of the desired operational range.
      // atof returns 0.0 for non-convertible strings.
      // If target_rate_str is not empty and target_rate is 0.0, it's likely a parse error.
      // However, the range check < 2.0 already covers 0.0.
      lv_msgbox_create(NULL, "Input Error", "Target rate must be between 2.0 and 400.0 kg/ha.", NULL, true);
      return;
  }
  if (grams_per_rev < 0.01) {
    lv_msgbox_create(NULL, "Seed Calibration", "Missing or invalid calibration", NULL, true);
    return;
  }
  if (gpr_fert < 0.01) {
    lv_msgbox_create(NULL, "Fert Calibration", "Missing or invalid calibration", NULL, true);
    return;
  }

  // TODO: Implement fertilizer flow calculation and control logic.
  // The following variables are available:
  // - gpr_fert (fertilizer grams/revolution, from calibration screen)
  // - fert_rate_input (lv_obj_t* for target fertilizer rate from main screen UI)
  // - A separate revs_per_sec calculation might be needed if fertilizer uses a different mechanism.
...
void create_ui() {
  ...
  lv_obj_add_event_cb(gps_input, [](lv_event_t *e) {
    gps_speed_kmph = atof(lv_textarea_get_text((lv_obj_t *)e->target));
  }, LV_EVENT_VALUE_CHANGED, NULL);

  // main_screen is already created and loaded above
  lv_obj_t *scr = main_screen; // Or lv_scr_act(), but main_screen is explicitly loaded
  ...
  lv_obj_t *seed_bar_lbl = lv_label_create(scr); // Changed variable name
  lv_label_set_text(seed_bar_lbl, "Seed");
  lv_obj_align_to(seed_bar_lbl, seed_bar, LV_ALIGN_OUT_BOTTOM_MID, 0, 5);
  ...
  lv_obj_t *fert_bar_lbl = lv_label_create(scr); // Changed variable name
  lv_label_set_text(fert_bar_lbl, "Fert");
  lv_obj_align_to(fert_bar_lbl, fert_bar, LV_ALIGN_OUT_BOTTOM_MID, 0, 5);
}
...
void create_calibration_screen() {
  ...
  lv_obj_t *seed_btn_lbl = lv_label_create(seed_btn); // Changed variable name
  ...
  lv_obj_t *fert_btn_lbl = lv_label_create(fert_btn); // Changed variable name
...
void setup() {
  ...
  lvgl_setup(); // Assuming this initializes LVGL subsystems like display and input drivers

  Preferences prefs; // Needs #include <Preferences.h>
  prefs.begin("calib", true); // Read-only mode for setup
  ...
  create_calibration_screen(); // Create it first
  create_ui(); // Create main UI, which will be loaded by default by lv_scr_load in create_ui

  // Load stored calibration times after UI is created so labels exist
  if (seed_ts > 0 && seed_time_label) { // Check if label exists
    char date_buf[16];
    strftime(date_buf, sizeof(date_buf), "%d/%m/%Y", localtime(&seed_ts)); // Needs #include <time.h>
    lv_label_set_text(seed_time_label, date_buf);
  }
  if (fert_ts > 0 && fert_time_label) { // Check if label exists
    char date_buf[16];
    strftime(date_buf, sizeof(date_buf), "%d/%m/%Y", localtime(&fert_ts)); // Needs #include <time.h>
    lv_label_set_text(fert_time_label, date_buf);
  }
}
