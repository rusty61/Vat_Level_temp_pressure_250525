					...
					*/
					...
					float computeVolumeVertical(float measuredH) {
						...
						if (fabsf(CAP) < 1e-3f) {  // Assuming CAP holds the slope angle for vertical tanks
							...
							Serial.print("Input measuredH (from outlet): ");
							Serial.print(measuredH);
							Serial.println(" mm");
							Serial.print("Input CAP (slope): ");
							Serial.print(CAP);
							Serial.println(" degrees");
							Serial.print("Input R (radius): ");
							Serial.print(R);
							Serial.println(" mm");
							Serial.print("Input H_MAX (wall height at outlet): ");
							Serial.print(H_MAX);
							Serial.println(" mm");
							...
							Serial.print("Calculated theta: ");
							Serial.print(theta);
							Serial.println(" rad");
							Serial.print("Calculated hDrop (total diametrical drop): ");
							Serial.print(hDrop);
							Serial.println(" mm");
							...
							for (int i = 0; i < N; i++) {
								...
								if (i == 0 || i == N / 2 || i == N - 1) {
									...
									Serial.print(y);
									Serial.println(" mm");
									Serial.print("    floorOffset_from_highest_point: ");
									Serial.print(floorOffset_from_highest_point);
									Serial.println(" mm");
									Serial.print("    floor_height_above_outlet: ");
									Serial.print(floor_height_above_outlet);
									Serial.println(" mm");
									Serial.print("    Raw liquid_col_H (measH - floor_H_abv_outlet): ");
									Serial.print(measuredH - floor_height_above_outlet);
									Serial.println(" mm");
									Serial.print("    Constrained liquid_col_H_above_floor_at_y: ");
									Serial.print(liquid_col_H_above_floor_at_y);
									Serial.println(" mm");
									Serial.print("    max_wall_H_above_floor_at_y: ");
									Serial.print(max_wall_H_above_floor_at_y);
									Serial.println(" mm");
									Serial.print("    sliceWidth: ");
									Serial.print(sliceWidth);
									Serial.println(" mm");
									Serial.print("    sliceVolume: ");
									Serial.print(sliceVolume);
									Serial.println(" mm^3");
								}
							}
							Serial.print("Calculated total vol_mm3 (sloped): ");
							Serial.println(vol_mm3);
					...
					void colorUptime() {
						...
						Serial.println("--- Preparing to send data to V42 (in colorUptime) ---");
						Serial.print("  Sending TempCorrection: ");
						Serial.println(TempCorrection);
						Serial.print("  Sending PLANT_WASH: ");
						Serial.println(PLANT_WASH);
						Serial.print("  Sending HOT_WATER: ");
						Serial.println(HOT_WATER);
						Serial.print("  Sending max_temp: ");
						Serial.println(max_temp);
						Serial.print("  Sending H_MAX: ");
						Serial.println(H_MAX);
						Serial.print("  Sending L: ");
						Serial.println(L);
						Serial.print("  Sending CAP: ");
						Serial.println(CAP);
						Serial.print("  Sending R: ");
						Serial.println(R);

						Blynk.virtualWrite(V42, TempCorrection, PLANT_WASH, HOT_WATER, max_temp, H_MAX, L, CAP, R, orientation, coneTYPE);
					...
					BLYNK_WRITE(V42) {
						...
						Serial.println("Received parameters from server for V42:");
						for (int i = 0; i <= 7; ++i) {  // V42 can have up to 8 params (0-7) for some plans
							Serial.print("  param[");
							Serial.print(i);
							Serial.print("]: ");
							Serial.println(param[i].asFloat());
						}
						...
						if (R > 1e-3f && H_MAX > 1e-3f) {  // Basic check before using in calculations (avoid zero/negative)
							...
							tankVolumeTotal = computeTankCapacity();
							Serial.printf("  Recalculated after V42 sync: refRaw=%.1f, scale=%.3f mm/count, totalVol=%.1f L\n",
														rawRefEstimated, heightScale, tankVolumeTotal);
						} else {
							Serial.println("  Skipping recalcs as R or H_MAX is zero/negative after V42 sync.");
						}
						Serial.println("--- End BLYNK_WRITE(V42) ---");
					}
